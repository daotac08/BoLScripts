...
function PluginOnLoad()
        ...
        Menu()
        VP = VPrediction()
        --ProdictQ = Prodict:AddProdictionObject(_Q, AutoCarry.PluginMenu.qRanger, 1350, 0.250, 101)
end
...
function KS()
    for i = 1, heroManager.iCount do
        ...
        if QAble and ValidTarget(Enemy, qRange, true) and Enemy.health <= getDmg("Q",Enemy,myHero)+getDmg("AD", Enemy, myHero) then
            CastQKS(Enemy)
        end
    end
end
...
AutoCarry.PluginMenu:addParam("qRanger", "Range - Boomerang Blade", SCRIPT_PARAM_SLICE, 875, 550, 1075, 0)
...
if QAble then
  ...
  if HitChance >= 2 and GetDistance(CastPosition) < AutoCarry.PluginMenu.qRanger then
...
PrintChat("<font color='#FF1493'> >> Snowstorm Sivir by Toy / VPred Version By Galaxix v1.0 Loaded ! <<</font>")
