...
AutoCarry Plugin - Evelynn 1.0 by Galaxix
...
function Combo()
  if Target then
                ...
                if (CountEnemyHeroInRange(EnemiesRange) >= MinimumEnemies) and GetDistance(Target) <= rRange then CastR(Target) end
...
function CastR(Target)
    if RREADY then
                local ultPos = GetAoESpellPosition(650, Target)
...
function CountEnemies(point, range)
    ...
    for j = 1, heroManager.iCount, 1 do
        ...
        if myHero.team ~= enemyhero.team and ValidTarget(enemyhero, rRange-400) then
...
function loadMain()
                ...
                qRange, wRange, eRange, rRange = 500, 0, 225, 650
...
function menuMain()
        Menu:addParam("sep", "-- Farm Options --", SCRIPT_PARAM_INFO, "")
               Menu:addParam("qFarm", "(Q) - Farm ", SCRIPT_PARAM_ONKEYTOGGLE, true, HK1)
                ...
                Menu:addParam("sKS", "Use Smart Combo KS", SCRIPT_PARAM_ONOFF, true, HK2)
                ...
                Menu:addParam("sep6", "-- Jungle Clear Options --", SCRIPT_PARAM_INFO, "")
                Menu:addParam("JungleE", "Use (E) In Jungle Clear", SCRIPT_PARAM_ONOFF, true)
                Menu:addParam("JungleQ", "Use (Q) In Jungle Clear", SCRIPT_PARAM_ONOFF, true)
...
function JungleClear()
	...
	if JungleMob ~= nil and not IsMyManaLow() then
		if Menu.JungleQ and GetDistance(JungleMob) <= qRange then CastSpell(_Q, JungleMob) end
        if Menu.JungleE and GetDistance(JungleMob) <= eRange then CastSpell(_E, JungleMob) end
	end
end
--end jungle clear
